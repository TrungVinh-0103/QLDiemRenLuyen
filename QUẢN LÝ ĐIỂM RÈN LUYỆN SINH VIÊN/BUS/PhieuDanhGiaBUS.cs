...
namespace QUẢN_LÝ_ĐIỂM_RÈN_LUYỆN_SINH_VIÊN.BUS
{
    public class PhieuDanhGiaBUS
    {
        ...
        public DataTable GetPhieuDanhGia(string maSV, string maHocKy, int nam, string v)
        {
            return pgDAO.GetPhieuDanhGia(maSV, maHocKy, nam);
        }

        public void AddMinhChung(MinhChung mc)
        {
        ...
        public DataTable GetPhieuDanhGiaByMaPhieu(int maPhieu)
        {
            ...
            try
            {
                return pgDAO.GetPhieuDanhGiaByMaPhieu(maPhieu);
            }
            catch (Exception ex)
            {
                Logger.LogError(ex.Message);
        ...
        public bool UpdatePhieuDanhGia(int maPhieu, PhieuDanhGia pg)
        {
            if (maPhieu <= 0)
            {
                throw new Exception("Mã phiếu không hợp lệ.");
            ...
            try
            {
                return pgDAO.UpdatePhieuDanhGia(maPhieu, totalScore, pg);
            }
            catch (Exception ex)
            {
                Logger.LogError(ex.Message);
